...
export class ActivityForm implements OnInit {
  ...
  private handleError(error: HttpErrorResponse): void {
    ...
    // περιγραφή. Η περιγραφή περιέχει τα συγκεκριμένα (σκεύασμα, ημερομηνίες)
    // που δεν μπορούμε να ανασυνθέσουμε εδώ.
    if (error.status === 400 && body?.code === 'ActivityInvalidArgument') {
      this.errorMessage.set(`Ο κανόνας του ημερολογίου δεν επιτρέπει την καταχώρηση: ${body.description}`);
      return;
    }
